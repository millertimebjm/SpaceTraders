...
public interface IShipCommandsHelperService
{
    ...
    Task<(Cargo?, Cooldown?)> Extract(Ship ship, Waypoint currentWaypoint, Waypoint miningWaypoint);
    ...
    Task<(Nav?, Fuel?)> NavigateToConstructionWaypoint(Ship ship, Waypoint currentWaypoint, Waypoint constructionWaypoint);
    ...
    Task<(Nav?, Fuel?)> NavigateToSurvey(Ship ship);
